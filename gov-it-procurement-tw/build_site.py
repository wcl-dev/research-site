...
{foot_line}<br>
資料：政府電子採購網 招標／決標公告（經 Twinkle AI pcc-tender 鏡像）· 廠商集團以商工登記董監事還原 · 2026 / 06<br>
定位：在公開資料範圍內描述政府資訊採購的市場結構<br>
...
# 共用：競爭指標說明 find-key、來源段
DISCIPLINE = ('<div class="find-key rv" style="margin-top:36px"><span class="kl">如何讀本研究的競爭指標</span>'
  '限制性招標、廠商集中、重複得標，反映的是<b>競爭的結構樣態</b>。其中限制性招標有其制度成因——'
  '《政府採購法》第 22 條列有適用情形（獨家、後續擴充、相容、緊急），資訊系統的後續維護與擴充常落入這些情形。'
  '本研究描述這些結構；個別案件的競爭情形，需投標家數等本資料未含的欄位方能判斷。</div>')

def sources(extra):
    return f"""<div class="src-grid">
<div class="src"><span class="src-k">主要資料 · 採購</span><b>政府電子採購網（web.pcc.gov.tw）招標／決標公告</b>（行政院公共工程委員會）。本研究透過 Twinkle AI（twinkle-hub）的 pcc-tender 鏡像查詢；決標公告約 8.6 萬筆，取其資訊類 13,339 筆，2015–2026。授權：政府網站資料開放宣告，須註明出處。</div>
...
ov_hero=hero(
  ...
  "資料：政府電子採購網（Twinkle AI 鏡像）· 廠商集團以商工登記董監事還原")
...
ov_body = (
 sec(1,"scope","母體與方法","母體與方法：什麼算「資訊採購」，以及兩道清洗",
   "本專題是「政府採購人格類型學」的資訊類延伸，沿用相同的資料源與研究方法。",
   '<div class="prose rv"><p>政府每年以公帑採購資訊系統與服務，這些系統承載稅務、健保、郵政、戶政、交通等核心業務的日常運作。本研究的資訊類母體（13,339 件、清洗後名目約 3,050 億）即是觀察這個市場結構的窗口。</p>'
   '<p>本研究的「資訊類母體」定義為：標的屬<b>財物或勞務</b>（排除工程類，以剔除捷運號誌、鐵路電氣化等系統工程）、且標案名稱命中資訊關鍵字（系統／軟體／資訊／資通／雲端／資安／網路／程式等）。得到 <b>13,339 件</b>資訊類決標，橫跨 2015–2026。原始資料各年份收錄密度不一，故跨年趨勢僅就「未經公開評選」這一十一年用字一致的欄位進行，並條件化於「公告金額以上」母體。</p></div>'
 ...
 + sec(3,"synthesis","綜述","綜述：市場結構與數位轉型、開源的關係",None,
   ...
   '<p>其二，採購結構在主管機關自身亦可見。政府將相當部分資訊業務委由自家捐助法人承作——工業技術研究院（91 案，跨 24 機關）、資訊工業策進會（70 案，跨 15 機關）；數位發展部自身金額最大的兩個承作對象，亦為資策會（24 案、16.7 億）與工研院。此一路徑未經一般公開競爭，屬供給端結構的一部分。</p>'
...
d_hero=hero(
  ...
  "資料：政府電子採購網（Twinkle AI 鏡像）· 招標方式以原始值判讀（欄名易誤導）")
...
d_body=(
 ...
 + sec(2,"rate","無公競爭採購","無公開競爭採購的比率高於全政府，並呈緩升",None,
   ...
   '</div></div>'
   '<div class="find-key"><span class="kl">說明</span>未經公開評選常見於原廠專屬、後續擴充等情形（採購法 §22 列有適用情形），故此比率反映的是競爭的結構特性。資訊類高於全政府，與資訊系統建置後維護、擴充多須由原建置者承作的特性一致。</div></div>'
 ...
 + sec(3,"pairs","兩種長年配對","機關層級的兩種長年配對","同樣是「多年由同一廠商承作」，其競爭性質可分為兩類；同一區分亦見於各機關的直接議價結構。",
   ...
   '<div class="crow"><div class="lbl">適用情形</div><div class="val">原廠專屬、後續擴充等，採購法 §22 列為得採單一來源</div></div>'
   '</div></article>'
   ...
   '</tbody></table>'
   '<p class="flag-note"><b>兩種型態並存：</b>中央研究院直接議價件數雖多，但分散於 110 家廠商（多為各自獨家的科研儀器與系統），接近前述「程序上未公開競爭、但廠商分散」一類；臺鐵、飛航服務總臺、智財局等則由極少數廠商承作（11、10、6 家），屬集中於少數既有廠商一類。兩類的差別在於競爭是否集中於少數既有廠商。</p></div></div>')
 + sec(4,"case","逐年重複的單一來源採購","逐年重複的單一來源採購：以中華郵政的大型主機為例","此例說明系統建置後，後續支出如何長期集中於原廠。",
   ...
   '</tbody></table>'
   '<p class="flag-note"><b>說明：</b>核心金融系統建置於 IBM z/OS 主機，後續授權、技術支援與硬體維護多須由原廠承作，形成每年約 3.6 億、九年累計約 33.8 億的單一來源採購。z/OS 為原廠專屬技術，後續授權與維護在技術上須由原廠承作；本例呈現的是建置選擇所帶來的長期轉換成本。若核心系統採開放標準或可攜架構，後續採購較有空間引入競爭。</p></div>')
 + sec(5,"limits","限制","限制",None,
   '<ul class="gaps rv">'
   '<li><b>個案競爭情形需另查。</b>未經公開評選、長年配對常有制度成因（採購法 §22 列有適用情形）；個別案件的競爭情形，需投標家數等本資料未含的欄位方能判斷。</li>'
...
b_hero=hero(
  ...
  "資料：政府電子採購網（Twinkle AI 鏡像）· 廠商縣市由登記地址解析 · 登記地非履約地")
...
a_body=(
 ...
 + sec(3,"largest","最大供應集團","規模明顯領先的供應集團為中華電信集團",None,
   ...
   '</tbody></table></div>'
   '<div class="find-key"><span class="kl">說明</span>政府資訊採購最大的單一供應方，是一個由政府為最大股東的集團，且其承載的多為稅務、健保、郵政、交通等核心系統。各機關的單一來源比不一（如公路總局較高、健保署多為公開競爭）；本文呈現的是供給端的結構事實。</div></div>')
 ...
 + sec(5,"foundation","法人承包","另一條供給管道：政府捐助法人","除商業廠商外，政府亦將相當部分資訊業務委由自家捐助法人承作。",
   ...
   '<p>值得一提的是，<b>數位發展部自身</b>金額最大的兩個資訊承作對象，正是資策會與工研院。委由捐助法人承作多以政策性計畫名義進行，未必經一般公開競爭。這構成在商業廠商的市場之外的另一條供給管道——「委由自家法人」，同樣不經開放競爭。就「為何開放競爭、開源或新進廠商不易擴大」而言，這是供給端結構的一部分。</p></div>')
...
c_hero=hero(
  ...
  "資料：政府電子採購網（Twinkle AI 鏡像）· 決標原則以欄位實際值判讀 · 與全政府基準對照")
